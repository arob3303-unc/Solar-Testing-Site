...
// NREL_API_KEY in your Vercel env vars. DEMO_KEY works for light testing (low rate limit).
//
...
export default async function handler(req, res) {
  ...
  const { state, zip, lat, lon } = req.body || {};
  ...
  if (!coords) {
    return res.status(200).json({ residentialCents: null, error: `No coordinates for ZIP "${zip}" / state "${state}"` });
  }
  ...
  try {
    ...
    const r = await fetch(url, {
      headers: { 'User-Agent': 'TTSS-Solar-Audit/1.0', 'Accept': 'application/json' }
    });
    const data = await r.json();

    if (!r.ok || data.errors?.length) {
      return res.status(200).json({ residentialCents: null, precision, place, error: data.errors?.[0] || `NREL API error (HTTP ${r.status})` });
    }
